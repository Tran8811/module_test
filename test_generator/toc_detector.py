...
# JSON schema ép output của LLM, thay cho việc dặn format bằng lời trong
# prompt. Được truyền qua response_format (OpenAI-compatible structured
# output, vLLM/SGLang hỗ trợ native) -- model bị ép sinh đúng cấu trúc này
# ở tầng decoding thay vì chỉ "được yêu cầu" trong prompt.
TOC_RESPONSE_FORMAT = {
    "type": "json_schema",
    "json_schema": {
        "name": "toc_levels",
...
def detect_heading_levels(lines: list[str]) -> dict[int, int]:
    ...
    for batch in _batch_lines_by_tokens(lines, TOC_BATCH_MAX_TOKENS):
        ...
        try:
            response = chat(
                prompt,
                max_tokens=needed_output_tokens,
